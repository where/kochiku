class Project < ActiveRecord::Base
  has_many :builds, :dependent => :destroy, :inverse_of => :project do
    ...
    def find_existing_build_or_initialize(ref, options)
      ...
      existing_build = Build.first(:joins => :project, :conditions => ["projects.repository_id = ? AND builds.ref = ?", proxy_association.owner.remote_repository.repository_id, ref], :readonly => false) unless options[:queue] == :ci
